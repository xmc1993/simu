...
//        return session.findListByQuery(modelCls, "from " + modelCls.getSimpleName());
    ...
    public cn.superid.jpa.query.Query<M> where() {
        return new cn.superid.jpa.query.Query<M>(modelCls, Model.getSession());
    }
    ...
    public cn.superid.jpa.query.Query<M> where(Session session) {
        return new cn.superid.jpa.query.Query<M>(modelCls, session);
    }
}
